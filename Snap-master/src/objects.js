...
SpriteMorph.prototype.blockTemplates = function (category) {
    ...
    if (cat === 'motion') {
        ...
        blocks.push(block('ensure'));
        blocks.push(block('result'));
        blocks.push(block('implies'));
        blocks.push(block('invariant'));
        blocks.push(block('old'));
        blocks.push(block('require' +
            '' +
            '' +
            ''));
...
function findvariables(variables,block){
    var top = block.topBlock();
    if(top.nextBlock()){
        variables = findvariableblock(top,variables)
    }
    return variables;
}

function findmethods(methods,block){
    var top = block.topBlock();
    if(top.nextBlock()){
        methods = findmethodsblock(top,methods)
    }
    return methods;
}

function findmethodsblock(block,methods){
    methods = block.findmethods(methods);


    if(block.nextBlock()){
        methods = findmethodsblock(block.nextBlock(),methods);
    }
    return methods;
}

function findvariableblock(block,variables){
    if (isBlock(block)) {
        variables = block.findvariables(variables);
    }
    if(block.nextBlock()){
        variables = findvariableblock(block.nextBlock(),variables);
    }
    return variables;
}
...
function removeduplicates(splittedvar){
    for (var i = 0; i < splittedvar.length; i++){
        for (var j = 0; j < splittedvar.length; j++){
            if(j!=i && splittedvar[i].localeCompare(splittedvar[j]) == 0){
                splittedvar.splice(i,1);
            }
        }
    }
    return splittedvar;
...
function declarevar(variables){
    var string = "";
    var splittedvar = variables.split(" ");
    var i = 0;
    splittedvar = removeduplicates(splittedvar);
    while (i<splittedvar.length-1){
        string = string.concat("var ");
        string = string.concat(splittedvar[i]);
...
function modifyvar(variables){
    var string = "";
    var splittedvar = variables.split(" ");
    var i = 0;
    splittedvar = removeduplicates(splittedvar);
    while (i<splittedvar.length-1){
        string = string.concat("modifies ");
        string = string.concat(splittedvar[i]);
...
function isverificationblock(block){
    if(block.blockSpec.localeCompare("ensures %b") == 0 || block.blockSpec.localeCompare("requires %b") == 0 || block.blockSpec.localeCompare("invariant %b") == 0 ){
        return true;
    }else{
        return false;
    }
...
function hasnestedblock(block){
    if(block.blockSpec.localeCompare("if %b %c") == 0 || block.blockSpec.localeCompare("if %b %c else %c") == 0 || block.blockSpec.localeCompare("repeat until %b %loop") == 0 || block.blockSpec.localeCompare("repeat %n %loop") == 0) {
        return true;
    }else{
        return false;
    }
...
class node{
    ...
    toBoogie(string){
        string = string.concat("i am an undifined to boogie method. \n");
        return string;
    }
    transform(){
        //my childs job;
...
class snapNode extends node{
    constructor(block) {
        super();
        this.methods = [];
        this.methods[0] = block;
        this.var = "";
        //this.var = variables.concat(findvariables(variables,block));
        this.methods = findmethods(this.methods,block);
        var i = 0;
        while (i < this.methods.length){
            this.childeren[i] = new methodNode(this.methods[i]);
            this.var = this.var.concat(this.childeren[i].var);
            i = i + 1;
        }
    }
    toBoogie(string) {
        string = string.concat(declarevar(this.var));
        var i = 0;

        while (i < this.childeren.length){
            string = this.childeren[i].toBoogie(string);
            i = i + 1;
        }
        return string;
    }
...
class methodNode extends node{
    constructor(block){
        super();
        var variables = "";
        this.topblock = block.topBlock();
        this.var = variables.concat(findvariables(variables,block));
        if (this.topblock.nextBlock()) {
            this.childeren[0] = new verificationNode(this.topblock);
            this.childeren[1] = new codeNode(this.topblock);
        }

    }
    toBoogie(string) {
        string = string.concat("procedure f() \n");
        if (this.childeren[0]) {
            string = this.childeren[0].toBoogie(string);
        }
        string = string.concat(modifyvar(this.var));
        string = string.concat("{ \n");
        if (this.childeren[1]) {
            string = this.childeren[1].toBoogie(string);
        }
        string = string.concat("} \n \n \n")
        return string;
    }
...
class verificationNode extends node{
    constructor(topblock) {
        super();
        this.blocks = [];
        this.verificationblocks = [];
        if (topblock){
            this.blocks[0] = topblock;
            var i = 0;
            while(this.blocks[i].nextBlock()){
                this.blocks[i+1] = this.blocks[i].nextBlock();
                i = i + 1;
            }
        }
        var i = 0;
        ...
        while (i < this.blocks.length){
            if (isverificationblock(this.blocks[i])){
                this.verificationblocks[counter] = this.blocks[i];
                counter = counter + 1;
            }
            i = i + 1;
        }
        i = 0;
        while (i < this.verificationblocks.length){
            this.childeren[i] = new blockNode(this.verificationblocks[i]);
            i = i +1;
        }
    }
    toBoogie(string) {
...
class codeNode extends node{
    constructor(topblock) {
        ...
        this.blocks = [];
        if (topblock){
            this.allblocks[0] = topblock;
            var i = 0;
            while(this.allblocks[i].nextBlock()){
                this.allblocks[i+1] = this.allblocks[i].nextBlock();
                i = i + 1;
            }
        }
        var i = 0;
        var counter = 0;
        while (i < this.allblocks.length){
            if (!isverificationblock(this.allblocks[i])){
                this.blocks[counter] = this.allblocks[i];
                counter = counter + 1;
            }
            i = i + 1;
        }
        i = 0;
        while (i < this.blocks.length){
            if (hasnestedblock(this.blocks[i])){
                this.childeren[i] = new loopNode(this.blocks[i]);
            } else {
                this.childeren[i] = new blockNode(this.blocks[i]);
            }
            i = i +1;
        }
    }
...
class nestedNode extends node{
    constructor(block) {
        super();
        this.top = block;
        this.childeren[0] = new verificationNode(this.top);
        this.childeren[1] = new codeNode(this.top);
    }
    toBoogie(string) {
        string = this.childeren[0].toBoogie(string);
        string = string.concat("{ \n");
...
class loopNode extends node{
    constructor(block) {
        super();
        this.block = block;
        this.inputs = this.block.inputs();
        this.nested = [];
        var i = 0;
        var c = 0;
        var n = 0;
        while (i < this.inputs.length){
            if (this.inputs[i].blockSpec){
                this.childeren[c] = new blockNode(this.inputs[i]);
                c = c + 1;
            } else {
                this.nested[n] = new nestedNode(this.inputs[i].evaluate());
                n = n + 1;
            }
            i = i + 1;
        }
    ...
    toBoogie(string) {
        if(this.block.blockSpec.localeCompare("if %b %c else %c") == 0){
            string = string.concat("if (");
            string = this.childeren[0].toBoogie(string);
            string = string.concat(")\n");
            string = this.nested[0].toBoogie(string);
            string = string.concat("else")
            string = this.nested[1].toBoogie(string);
            string = string.concat("\n")
        } else if(this.block.blockSpec.localeCompare("if %b %c") == 0) {
            string = string.concat("if (");
            string = this.childeren[0].toBoogie(string);
            string = string.concat(")\n");
            string = this.nested[0].toBoogie(string);
            string = string.concat("")
        } else if(this.block.blockSpec.localeCompare("repeat until %b %loop") == 0) {
            ...
            string = string.concat(this.nested[0].toString())
            //string = string.concat("Undefined loop block \n");
        }
        return string;
    }
}
...
class blockNode extends node{
    constructor(block) {
        ...
        var i = 0;
        while (i < this.inputs.length){
            this.childeren[i] = new blockNode(this.inputs[i]);
            i = i + 1;
        }
    }
    toBoogie(string) {
        if(this.block instanceof InputSlotMorph) {
            string = string.concat(this.block.evaluate());
        }

        else if(this.block.blockSpec.localeCompare("ensures %b") == 0){
            string = string.concat("ensures ")
            string = this.childeren[0].toBoogie(string);
            string = string.concat(";\n")
        }
        else if(this.block.blockSpec.localeCompare("call %repRing %inputs") == 0){
            string = string.concat("call ")
            string = this.childeren[0].toBoogie(string);

        }
        else if(this.block.blockSpec.localeCompare("%rr %ringparms") == 0){
            string = string.concat(this.block.parts()[0].evaluate().blockSpec);
            string = string.concat("(")
            string = string.concat(")")
            //string = this.childeren[0].toBoogie(string);
        ...
        else if(this.block.blockSpec.localeCompare("%b => %b") == 0){
            string = string.concat("(");
            string = this.childeren[0].toBoogie(string);
            string = string.concat("==> ")
            string = this.childeren[1].toBoogie(string);
        ...
        else if(this.block.blockSpec.localeCompare("when %greenflag clicked") == 0){

        }
        else if(this.block.blockSpec.localeCompare("invariant %b") == 0){
            string = string.concat("invariant ")
            string = this.childeren[0].toBoogie(string);
            string = string.concat(";\n")
        }
        else if(this.block.blockSpec.localeCompare("requires %b") == 0){
            string = string.concat("requires ")
            string = this.childeren[0].toBoogie(string);
            string = string.concat(";\n")
        }
        else if(this.block.blockSpec.localeCompare("result") == 0){
            string = string.concat("result ")
        }

        else if (this.block.blockSpec.localeCompare("%b and %b") == 0) {
            string = string.concat("(");
            string = this.childeren[0].toBoogie(string);
            string = string.concat(" && ");
            string = this.childeren[1].toBoogie(string);
        ...
        else if (this.block.blockSpec.localeCompare("not %b") == 0) {
            string = string.concat("(");
            string = string.concat(" !");
            string = this.childeren[0].toBoogie(string);
            string = string.concat(")");
        ...
        else if (this.block.blockSpec.localeCompare("%b or %b") == 0) {
            string = string.concat("(");
            string = this.childeren[0].toBoogie(string);
            string = string.concat(" || ");
            string = this.childeren[1].toBoogie(string);
        ...
        else if (this.block.blockSpec.localeCompare("%s < %s") == 0) {
            string = string.concat("(");
            string = this.childeren[0].toBoogie(string);
            string = string.concat(" < ");
            string = this.childeren[1].toBoogie(string);
        ...
        else if (this.block.blockSpec.localeCompare("change %var by %n") == 0) {
            string = this.childeren[0].toBoogie(string);
            string = string.concat(" := ");
            string = this.childeren[0].toBoogie(string);
            string = string.concat(" + ");
        ...
        else if (this.block.blockSpec.localeCompare("set %var to %s") == 0) {
            string = this.childeren[0].toBoogie(string);
            string = string.concat(" := ");
            string = this.childeren[1].toBoogie(string);
            string = string.concat(";\n ");
        ...
        else if (this.block.blockSpec.localeCompare("%n + %n") == 0) {
            string = string.concat("(");
            string = this.childeren[0].toBoogie(string);
            string = string.concat(" + ");
            string = this.childeren[1].toBoogie(string);
        ...
        else if (this.block.blockSpec.localeCompare("report %s") == 0) {
        ...
        else if (this.block.blockSpec.localeCompare("old %n") == 0) {
            string = string.concat("old(");
            string = this.childeren[0].toBoogie(string);
            string = string.concat(")");

        }
        else {
            string = string.concat(this.block.blockSpec);
        }
...
StageMorph.prototype.fireVerifyEvent = function(){
    var string = '//this is the text file to verify the snap code\n';
    var variables = "";
    this.children.concat(this).forEach(function (morph) {
        if (isSnapObject(morph)) {
            morph.allHatBlocksFor('__shout__go__').forEach(function (block) {
                ast = new snapNode(block);
                ast.transform();
                string = ast.toBoogie(string);
            });
        }
    });
    return string
...
StageMorph.prototype.makestring = function(string){
    string = string.concat("\n im a not yet named block");
    return string
}
